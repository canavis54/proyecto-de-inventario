...
//muestra la tabla de la base de datos 
    function renderItems(items) {
        ...
        items.forEach(item => {
            ...
            const deleteButton = createButton('Eliminar', () => openDeleteModal(item.id));
            ...
            actionsCell.append(editButton, deleteButton, editQuantityButton);
        });
    }
...
//funcion para abrir el modal y limpiar el registro para  agregra 
...
//funcion prar abrir el modal de la cantidad
    function openEditQuantityModal(item) {
        ...
        resultQuantity.textContent = item.cantidad;
...
//funcion para eliminar las cuentas 
    function deleteItem() {
        if (itemToDelete !== null) {
            fetch(`/api/items/${itemToDelete}`,{ method: 'DELETE' })
                .then(response => {
                    if (response.ok) {
                        fetchItems();
                        closeModal();
    ...
    window.addEventListener('click', (event) => {
        if (event.target === modal || event.target === editQuantityModal || event.target === deleteUserModal) {
            closeModal();
        }
    });
